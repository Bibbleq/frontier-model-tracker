...
- `status.json` is a machine-readable summary: totals, structured build warnings, coverage gaps and lag answerability. It carries no timestamp, so rebuilds stay byte-identical.
...
- `current-state.csv` is the last known lifecycle for each model and surface, derived on every build. Read `state_is_terminal` before presenting `lifecycle` as current: the dataset records events, not states, and only a terminal stage is safe to read as present tense. `open_questions` counts unresolved backlog items for that pair, and `known_as_of` is the research cutoff.
